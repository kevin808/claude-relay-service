...
async function handleImageStreamGenerateContent(req, res) {
  ...
  try {
    ...
    if (!account) {
      return res.status(404).json({ error: { message: 'Selected account not found.', type: 'invalid_request_error' } });
    }
    ...
    streamResponse.on('error', (error) => {
      ...
      if (!res.headersSent) {
        res.status(500).json({ error: { message: error.message || 'Stream error', type: 'api_error' } });
      }
    });
    ...
    if (!res.headersSent) {
      res.status(500).json({ error: { message: error.message || 'Internal server error', type: 'api_error' } });
    }
  } finally {
    if (abortController) {
